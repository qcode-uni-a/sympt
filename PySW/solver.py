...
class EffectiveFrame:
    ...
    def solve(self, max_order=2, full_diagonalization=False, mask=None):
        ...
        if full_diagonalization and mask is not None:                                                        # Check if the mask is used in the full diagonalization mode
            ...
            print('The mask is not used in the full diagonalization mode and it will be ignored')

        if self.V_old is not None and not do_regular_SW:                                                  # Check if the perturbative interaction will be added to the full Hamiltonian
            print('The perturbative interaction will be added to the full Hamiltonian')
            self.H_old = (self.H_old + self.V_old).expand()                                                     # Add the perturbative interaction to the Hamiltonian
            self.V_old = S.Zero                                                                                  # Set the perturbative interaction to zero

        mask = mask + mask.hermitian() + Blocks() if mask is not None else None                               # Add the Hermitian conjugate and the blocks to the mask. This ensures that the mask is Hermitian

        factorials = [sp_Rational(1, sp_factorial(k)) for k in range(0, max_order + 1)]                 # Compute the factorials for the perturbative orders

        Ss = {}                                                                                            # Initialize the dictionary to store the anti-Hermitian operator S for each order
        ...
        if do_regular_SW:           # If the regular Schrieffer-Wolff transformation is used
            Hs = Hs_aux                                                                                    # Set the Hamiltonians to the perturbative expressions
            Vs = get_perturbative_expression(self.V_old, self.__structure , self.subspaces)                # Compute the perturbative expression for the perturbative interaction
            if Vs.get(0) is not None:
                raise ValueError('The zeroth order of the perturbative interaction should be zero')        # Check if the zeroth order of the perturbative interaction is zero
        else:                    # If the full diagonalization or mask routine is used
            Vs = {}
            Hs = {}
            for h_order, h_expr in Hs_aux.items():                                                        # Iterate over the perturbative expressions for the Hamiltonian
                if h_order == 0:                                                                     # If the order is zero
                    new_vk, new_hk = separate_diagonal_off_diagonal(h_expr)                                # Separate the diagonal and off-diagonal terms of the Hamiltonian
                    if new_vk.expr.shape[0] != 0:                                                       # If the diagonal terms are not zero
                        raise ValueError('The zeroth order of the Hamiltonian should be diagonal')        # Check if the zeroth order of the Hamiltonian is diagonal
                    Hs[h_order] = new_hk                                                            # Set the Hamiltonian to the off-diagonal terms
                    continue

                if mask is not None:                                                                  # If the mask is used
                    new_vk, new_hk = mask.apply_mask(h_expr)                                         # Apply the mask to the perturbative expression
                elif full_diagonalization:                                                          # If the full diagonalization routine is used
                    new_vk, new_hk = separate_diagonal_off_diagonal(h_expr)                           # Separate the diagonal and off-diagonal terms of the Hamiltonian
        ...
        for order in trange(1, max_order + 1, desc='Solving for each order'):                        # Iterate over the perturbative orders

            set_of_keys = partitions(order)                                                           # Compute the partitions for the perturbative order
            B_k = Expression()                                                                     # Initialize the operator B_k for the perturbative order

            for key in set_of_keys[:-1]:                                                         # Iterate over the partitions. Eliminate the last partition because it is the term [H0, S] and it is used to compute the operator S

                if len(key) == 1:
                    # Does not deppend of full_diagonalization neither on mask
                    Vk = Vs.get(key[0], Expression())
                    B_k += Vk                                                              # Add the perturbative interaction to the operator B_k (Equation to solve)
                    H_final += Hs.get(key[0], Expression())                               # Add the perturbative Hamiltonian to the final Hamiltonian
                    continue

                nestedness = len(key) - 1                                                   # Compute the nestedness of the partition
                
                if do_regular_SW:                                                        # If the regular Schrieffer-Wolff transformation is used
                    B_k = (B_k + nest_commute(key, nestedness % 2 == 0) * factorials[nestedness]).simplify()    # Compute the nested commutator for the regular Schrieffer-Wolff transformation
                    H_final = (H_final + nest_commute(key, nestedness % 2 != 0) * factorials[nestedness]).simplify()    # Compute the nested commutator for the regular Schrieffer-Wolff transformation
                else:                                                              # If the full diagonalization or mask routine is used
                    new_commutator_odd = nest_commute(key, nestedness % 2 != 0) * factorials[nestedness]    # Compute the nested commutator for the full diagonalization or mask routine
                    new_commutator_even = nest_commute(key, nestedness % 2 == 0) * factorials[nestedness]   # Compute the nested commutator for the full diagonalization or mask routine
                    new_commutator = (new_commutator_odd + new_commutator_even).simplify()              # Compute the nested commutator for the full diagonalization or mask routine
                    if mask is not None:                                                           # If the mask is used
                        new_bk, new_hf = mask.apply_mask(new_commutator)                      # Apply the mask to the nested commutator
                    elif full_diagonalization:                                                 # If the full diagonalization routine is used
                        new_bk, new_hf = separate_diagonal_off_diagonal(new_commutator)      # Separate the diagonal and off-diagonal terms of the nested commutator
                    B_k = (B_k + new_bk).simplify()                                         # Add the nested commutator to the operator B_k
    ...
    def rotate(self, expr, max_order=None, return_form=True):
        ...
        for order in trange(1, max_order + 1, desc='Rotating for each order'):
            ...
            for key in set_of_keys:     # Iterate over the all the partitions. Do not eliminate any partition.
                if len(key) == 1:
                    result += Os.get(key[0], Expression())
                    continue

                nestedness = len(key) - 1
                result += (nest_commute(key) * factorials[nestedness]).simplify()
    ...
    def __str__(self):
        ...
        subspaces_finite = [[subspace.name, 'Finite', f'{subspace.dim}x{subspace.dim}'] for subspace in self.subspaces if subspace.name != 'finite_pysw_built_in_function'] if self.subspaces is not None else []
        subspaces_infinite = [[subspace.as_ordered_factors()[1].name, 'Bosonic', '∞'] for subspace in self.__structure.keys()]

        subspaces_info = tabulate(subspaces_headers + subspaces_finite + subspaces_infinite, headers='firstrow', tablefmt='rounded_grid', stralign='center')
